...
$query = 'SELECT 
            workers.name, 
            count(*) 
        from documents inner join workers on documents.worker = workers.id group by workers.name;';
...
for ($i = 0, $y1 = $imageHeight, $x1 = $rowInterval / 2; $i < count($rows); $i++) {
    ...
    imagettftext($img, 14, 90, $x1 - 5, $height - ($height - $imageHeight), 5, realpath('.') . "/roboto.ttf", $readers[$i]);
    $x1 = $x2 + $rowInterval;
}
