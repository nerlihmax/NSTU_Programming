...
int* getPrimeNumbers(int min, int max, int* count)
{
	if (max < 2)
		return NULL;
	...
	int finalCount = 0;
	for (int i = minBorder + 1; i < curCount; i++, finalCount++)
		primeNumbers[finalCount] = primeNumbers[i];

	primeNumbers = (int*)realloc(primeNumbers, sizeof(int) * finalCount);
	*count = finalCount;
	return primeNumbers;
}
...
{
	int primesCount;
	int* primes = getPrimeNumbers(number1, number2, &primesCount);
...
__global__ void sieveNumbers(int* primes, int primesStartCount, int min, int max, int elemsPerThread)
{
...
__global__ void findMinMaxCuda(int* primes, int primesCount, int min, int max, int elementsPerThread)
{	
	int blockId = blockIdx.x + blockIdx.y * gridDim.x;
	int id = blockId * THREADS_PER_BLOCK + threadIdx.x;
	if (id == 0) { *d_min = {INT_MAX, -1, -1}; *d_max = {INT_MIN, -1, -1};}	
	
	Result myMin = {INT_MAX, -1, -1}, myMax = {INT_MIN, -1, -1};

    for (int i = id * elementsPerThread; i < id * elementsPerThread + elementsPerThread && i < primesCount; i++)
    {
        int number = primes[i];
	...
	if (myMin.value != INT_MAX)
		atomicMin(&(d_min->value), myMin.value);
	if (myMax.value != INT_MIN)
		atomicMax(&(d_max->value), myMax.value);
...
void getNumberOfBlocksAndThreads(int elemsCount, dim3* blocks, dim3* threads)
{
	...
	_threads.x = (elemsCount < THREADS_PER_BLOCK ? elemsCount : THREADS_PER_BLOCK);
...
{
	int primesCount;
	int* primes = getPrimeNumbers(0, sqrt(number2), &primesCount);

	int theoreticalPrimesCount = ((number2 - number1) / log(sqrt(number2))) * 1.2;
	...
	cudaMemcpy(d_primes,primes, primesCount * sizeof(int),cudaMemcpyHostToDevice);
	...
	sieveNumbers << <blocks, threads >> > (d_primes, primesCount, number1, number2, elemsPerThread);
	...
	getNumberOfBlocksAndThreads(primesCount / elementsPerThread + 1, &blocks, &threads);
	
	findMinMaxCuda<<<blocks, threads>>>(d_primes, primesCount, number1, number2, elementsPerThread);
	cudaDeviceSynchronize();
...
{
	if (argc < 3) { printf("Args: number#1, number#2\n"); return -1; }
	
	int number1 = strtol(argv[1], NULL, 10);
	int number2 = strtol(argv[2], NULL, 10);
	
	if (number2 < number1) { printf("Condition: number1 < number2\n"); return -1;  }

	auto execute = [number1, number2](void(*func)(int, int, Result&, Result&), Result& min, Result& max)
	{        
		...
		printf("min value: \e[32m%d\e[0m = %d * %d (from %d)\n", min.value,
               min.number / min.dozens, min.number % min.dozens, min.number);
        printf("max value: \e[32m%d\e[0m = %d * %d (from %d)\n", max.value,
    ...
    execute(solveSequential, min2, max2);

    bool isEqual = (min1 == min2) && (max1 == max2);

    cout << "\nResults are " << (isEqual ? GREEN : RED)
         << (isEqual ? "equal" : "different") << PLAIN << endl;
