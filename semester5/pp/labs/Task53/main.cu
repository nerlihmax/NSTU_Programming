...
__global__ void sieveNumbers(int* primes, int primesStartCount, int min,
                             int max, int elemsPerThread) {
...
void solveParallel(int number1, int number2, Result& h_min, Result& h_max) {
    ...
    sieveNumbers<<<blocks, threads>>>(d_primes, primesCount, number1, number2,
                                      elemsPerThread);
    ...
    getNumberOfBlocksAndThreads(primesCount / elementsPerThread + 1, &blocks,
                                &threads);
...
int main(int argc, char** argv) {
    ...
    auto execute = [number1, number2](void (*func)(int, int, Result&, Result&),
